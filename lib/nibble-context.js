...
import { PROJECTS_BRAIN as ALL_PROJECTS } from './projects.js';

/* Benched projects are still written out in full in the brain, but they are not
   on the site right now, so Nibble must not see them at all. Filtering once here
   keeps every representation below consistent: they drop out of the index, the
   details map, the valid id set for open_project, and retrieval matching. */
const PROJECTS_BRAIN = ALL_PROJECTS.filter((p) => !p.hidden);
...
/* Longest-first so a multi-word alias wins over a stray single word. */
